function new_cell_pat=norm_fun(cell_pat)
new_cell_pat=cell(max(size(cell_pat)),1);   %output pattern cell array
pat=[];          %#ok old pattern
...
%normalization loop
for p=1:max(size(cell_pat))
    pat=cell_pat{p};             %old pattern 
    ...
    if(row_cnt > 8)
        ...
        %assertion process
        if row_cnt==sum(sa)
            disp('success minification row');
        else
            disp('fail minification row');
        end
    ...
    new_cell_pat{p}=new_pat; %dimensions are 1*(number of patterns) and by detailed are 8*(number_of_patterns *8)
    figure
    subplot(121)
    imshow(pat);
    title({'the old pattern "berfore normalization"';[int2str(size(pat,1)) '*' int2str(size(pat,2))]},'Color','y');
    subplot(122)
    imshow(new_pat);
    title({'the new pattern "after normalization"';[int2str(size(new_pat,1)),'*',int2str(size(new_pat,2))]},'Color','b');
end
end